...
import edu.handong.csee.isel.bytecodeparser.asm.jasm.OpcodeTables;
import edu.handong.csee.isel.bytecodeparser.asm.jasm.Tables;
...
import static edu.handong.csee.isel.bytecodeparser.asm.jasm.OpcodeTables.opcode;
import static edu.handong.csee.isel.bytecodeparser.asm.jasm.Tables.AttrTag.ATT_RuntimeInvisibleTypeAnnotations;
import static edu.handong.csee.isel.bytecodeparser.asm.jasm.Tables.attrtag;
import static edu.handong.csee.isel.bytecodeparser.asm.jasm.Tables.basictype;
import static edu.handong.csee.isel.bytecodeparser.asm.jdis.Utils.commentString;
...
public class CodeData extends Indenter {
    ...
    public void read(DataInputStream in, int codeattrlen) throws IOException {
        ...
        for (int k = 0; k < nattr; k++) {
            ...
            if (name_const != null && name_const.tag == ConstantPool.TAG.CONSTANT_UTF8) {
                ...
                // process the attr
                Tables.AttrTag attrtag = attrtag(attrname);
                switch (attrtag) {
    ...
    private int checkForLabelRef(int pc) {
        //         throws IOException {
        int opc = getUbyte(pc);
        OpcodeTables.Opcode opcode = opcode(opc);
        switch (opcode) {
...
// TraceUtils.traceln("****** [CodeData.printInstr]: got an '" + opcode.parseKey() + "' [" + opc + "] instruction ****** ");
        switch (opcode) {
            ...
            case opc_newarray:
                ...
                Tables.BasicType type = basictype(tp);
                switch (type) {
                    case T_BOOLEAN:
                        out.print(" boolean");
                        break;
